...
class Forum:
	...
	def login(self):
		r = requests.get(Forum.LOGIN_URL, headers = Forum.HEADERS )
		...
		r = requests.post(Forum.LOGIN_SUBMIT_URL, params=params, headers = Forum.HEADERS )
	...
	def display(self, page=1):
		r = requests.get(Forum.DISPLAY_URL % page, cookies=self.cookies, headers = Forum.HEADERS)
	...
	def search(self, title, page=1):
		r = requests.get(Forum.SEARCH_URL % (title, page), cookies=self.cookies, headers = Forum.HEADERS)
                pdb.set_trace()
		page = BeautifulSoup(r.content, "lxml", from_encoding="gb18030")
		pdb.set_trace()
		pages = 1
	...
	def viewthread(self, tid, page=1, authorid=None):
		...
		r = requests.get(Forum.THREAD_URL % (tid, page) + author_only, cookies=self.cookies, headers = Forum.HEADERS)
		page = BeautifulSoup(r.content, "lxml", from_encoding="gb18030")
	...
	def reply(self, msg, tid):
		...
		r = requests.get(reply_url, cookies=self.cookies, headers = Forum.HEADERS)
		...
		r = requests.post(reply_url + '&replysubmit=yes', params=params, cookies=self.cookies, headers = Forum.HEADERS )
		page = BeautifulSoup(r.content, "lxml", from_encoding="gb18030")
...
class DTerm:
	...
	def turn_image(self, url):
		...
		r = requests.get(url, headers = Forum.HEADERS)
